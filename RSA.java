...
class RSA{
  ...
  */
  ...
  public BigInteger[] desifrarCodificacion(BigInteger[] x, BigInteger j, BigInteger n){
    ...
    for (int i=0;i<codificacionFinal.length ;i++ ) {
        //auxiliar=k.intValue();
        codificacionFinal[i]=(potencia(numero,j,UNO)).mod(n);
  ...
  public static BigInteger potencia(BigInteger base, BigInteger exponente, BigInteger acumulador){
    if(exponente.compareTo(CERO)==0){
      return acumulador;
    }else{
      return potencia(base,exponente.add(MENOSUNO), acumulador.multiply(base));
    }
  }
  ...
  public static void main(String[] args) {
    ...
    BigInteger exponente = new BigInteger("2");
    System.out.println(potencia(base,exponente,UNO));
    /*String mensaje="hola";
